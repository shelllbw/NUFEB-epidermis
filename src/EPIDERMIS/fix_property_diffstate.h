...
FixStyle(skin/property/diffstate,FixPropertyDiffstate)

#else

#ifndef LMP_FIX_PROPERTY_DIFFSTATE_H
#define LMP_FIX_PROPERTY_DIFFSTATE_H

#include "fix_property.h"

namespace LAMMPS_NS {

class FixPropertyDiffstate : public FixProperty {
 public:

  FixPropertyDiffstate(class LAMMPS *, int, char **);
  virtual ~FixPropertyDiffstate();
 ...
 protected:
  ...
  int ical;
  double r_max;   // differentiation rate
  double ks;
  int group_flag;
  int *groupbits;
  int ngroups;
